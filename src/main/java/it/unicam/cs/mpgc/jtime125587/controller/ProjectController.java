...
public class ProjectController {
    ...
    public void add(@NonNull Project project) {
        HibernateUtil.runInTransaction((Consumer<Session>) session -> session.persist(project));
    }

    public void delete(@NonNull Project project) {
        HibernateUtil.runInTransaction((Consumer<Session>) session -> session.remove(project));
    }
    ...
    public List<String> getProjectNames() {
        return HibernateUtil.runInTransaction((Function<Session, List<String>>) session ->
                session.createQuery("select name from Project", String.class)
                        .getResultList());
    }

    public Project getProjectByName(String name) {
        return HibernateUtil.runInTransaction((Function<Session, Project>) session ->
